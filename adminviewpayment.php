...
$end=$_SESSION["end"] ;
if($start!=""and$end!=""){
    $sql = " SELECT reservation.payment_date as pd,SUM((reservation.return_date-reservation.reservation_date+1)*car.price) as price
     FROM reservation NATURAL JOIN car 
where  payment_date>='$start' and payment_date<='$end' and payment=1 
GROUP BY(reservation.payment_date) ";
}
else{
    $sql = " SELECT reservation.payment_date as pd,SUM((reservation.return_date-reservation.reservation_date+1)*car.price) as price
    FROM reservation NATURAL JOIN car where payment=1 GROUP BY(reservation.payment_date); ";
}
